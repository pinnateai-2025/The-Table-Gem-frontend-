...
const MugProducts = () => {
  ...
  // Products passed from DrinkwareProduct
  const [products, setProducts] = useState(location.state?.products || []);
  const API_URL = process.env.REACT_APP_API_URL;

  // Fetch Mug products if products not passed in state
  useEffect(() => {
    if (!products.length && API_URL) {
      fetch(`${API_URL}/products?category=Mug`)
        .then((res) => {
        ...
        .catch((err) => console.error("Error fetching Mug products:", err));
    }
        ...
        {products.length > 0 ? (
          products.map((product) => (
            ...
            >
              {/* Image Section */}
              <div
                className="w-[360px] h-[360px] max-[1024px]:w-[220px] max-[1024px]:h-[200px] max-[768px]:h-[250px] max-[500px]:w-[120px] max-[500px]:h-[120px] max-[375px]:w-[100px] max-[375px]:h-[100px] max-[320px]:w-[90px] max-[320px]:h-[90px] overflow-hidden rounded-md relative group cursor-pointer"
